...
public class SvgMapView extends View {
    ...
    Thread loadMapThread = new Thread() {
        ...
        public void run() {
            ...
            try {
                DocumentBuilder builder = DocumentBuilderFactory.newInstance().newDocumentBuilder();//拿到DocumentBuilder
                Document xml = builder.parse(getResources().openRawResource(R.raw.taiwan));//解析xml文件
                Element data = xml.getDocumentElement();//获取其根标签
                NodeList list = data.getElementsByTagName("path");//根据子标签进行查找，返回的是一个list集合
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (provinceEntityList != null && provinceEntityList.size() > 0) {
                Log.e(TAG, "provinceEntityList = " + provinceEntityList.size());
                postInvalidate();
            }
        }
    };
